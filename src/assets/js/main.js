...
!(function (d, c, l, u, g, s) {
  1 !== parseInt(l.msDoNotTrack || d.doNotTrack || l.doNotTrack, 10) &&
    d.addEventListener("load", function () {
      ...
      var t,
        ...
        o =
          ...
          (c.uid = c.uid || Math.random() + "." + Math.random()) +
          "&v=1&tid=" +
          galite.UA +
          "&dl=" +
          s(g.location.href) +
        ...
        a = {
          dt: [g.title],
          sd: [u.colorDepth, "-bit"],
          sr: [u.availHeight, "x", u.availWidth],
      ...
      (n = function (e, t) {
        ...
        return function () {
          var t =
            ...
            "&z=" +
            new Date().getTime();
          if (l.sendBeacon) l.sendBeacon(t);
          else
            try {
        ...
        setTimeout(n("pageview", null), 100),
        d.addEventListener(
          "unload",
          n("timing", {
            utc: "JS Dependencies",
            utv: "unload",
            utt: new Date().getTime() - e,
          })
        );
    });
})(window, localStorage, navigator, screen, document, encodeURIComponent);
var galite = galite || {};
galite.UA = "UA-17015176-6";
      ...
      getPhotos: function (e) {
        ...
        a.fetch(e).done(function (e) {
          ...
          e.data && e.data.length
            ? a.display(e)
            : t.error(
                "Spectragram.js - Error: " + s + " does not have photos."
              );
        });
      },
        ...
        if (
          ...
          for (var u = 0; u < c; u++)
            "small" === h
              ...
              i ? p.push(a) : p.push(t(this.options.wrapEachWith).append(a));
          this.$elem.append(p);
        }
...
/* Moralis init code */
const serverUrl = "https://c4ms4vh6woxz.usemoralis.com:2053/server";
const appId = "nntHVo5v8bBKaIFwrF0upUlvLwBa2rZ8t8hy9hlX";
...
async function login() {
  ...
  if (!user) {
    user = await Moralis.authenticate({
      signingMessage: "Just a normal signin request",
    })
      .then(function (user) {
...
async function mint() {
  ...
  const options = {
    chain: "mainnet",
    contractAddress: "0x2ddD77B20F0ee65bb68dc5DdE863135350ea3e8B",
    functionName: "sendTipForNFT",
    abi: ABI,
    msgValue: Moralis.Units.ETH(tip),
    params: {},
  };
  const allowance = await Moralis.executeFunction(options);
  await logOut();
  alert("Thank you for your Tip, Your NFT is minting");
}
